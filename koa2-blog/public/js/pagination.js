function pagination (data, callback) {
  ...
  function pageAction (dataPage) {
    ...
    if (dataPage > 1) {
      ...
      if (data.first) {
        i_html += '<a data-page="1" href=\"javascript:void(0);\">棣栭〉</a>'
      }
    }
    if (dataPage >= 5) {
      for (var i = 1; i <= 2; i++) {
    ...
    if (endPage + 1 < data.totalPage) {
      ...
      if (data.last) {
        i_html += '<a data-page="' + data.totalPage + '" href=\"javascript:void(0);\">灏鹃〉</a>'
      }
    ...
    if (data.jumpBtn && data.totalPage >= 1) {
      i_html += '鍓嶅線<input id="pageInput" type="text" />椤� <span id="inputGo">纭畾</span>'
    }
